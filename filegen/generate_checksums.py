        ...
        for line in f:
            ...
            if cs in checksum_map:
                if name != checksum_map[cs]:
                    logger.warn(
                        "Found hash collision: {} with {} -> 0x{:08X}".format(
                            name, checksum_map[cs], cs
                        )
                    )
                else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        "Generate checksums from strings in a file, creating a c header files with #defines of the checksums."
    )
    parser.add_argument(
        "--input-file",
        metavar="FILE",
    ...
    logger.info(
        "Generated '{}', containing {} checksums".format(
            args.output_file, len(checksum_map)
        )
    )
