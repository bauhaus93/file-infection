...
bool parse_instruction(void *addr, Instruction *instr) {
  if (addr != NULL && instr != NULL) {
    ...
    instr->operand_size = get_operand_size(instr->prefix, instr->prefix_count);
    ...
    if (instr->opcode_size == 0 || instr->opcode_size > 3) {
      ...
      instr->end =
          (void *)BYTE_OFFSET(instr->start, calculate_instruction_size(instr));
...
static void handle_modrm(Instruction *instr) {
  if (has_modrm(instr)) {
    instr->modrm = (uint8_t *)BYTE_OFFSET(instr->opcode, instr->opcode_size);
...
static void handle_opcode_extension(Instruction *instr) {
  if (has_opcode_extension(instr)) {
    uint8_t additional_immediate = get_opcode_extension_immediate_size(instr);
