...
static uint8_t functions_in_boundaries(void);
...
int main(int argc, char **argv) {
    ...
    PRINT_DEBUG("code_size: %.2fkiB",
                (float)((uint8_t *)code_end - (uint8_t *)code_begin) / 1024.);
    uint8_t in_boundaries = functions_in_boundaries();
    PRINT_DEBUG("functions in boundaries: %s", in_boundaries ? "yes" : "no");
    if (!in_boundaries) {
        PRINT_DEBUG("not all functions in boundaries, aborting");
        return 2;
    }
...
static uint8_t functions_in_boundaries(void) {
    return !IN_BOUNDARIES(main) && !IN_BOUNDARIES(functions_in_boundaries) &&
           IN_BOUNDARIES(checksum) && IN_BOUNDARIES(init_data) &&
           IN_BOUNDARIES(fill_function_list) &&
           IN_BOUNDARIES(get_function_by_checksum) &&
           ...
           IN_BOUNDARIES(memcp) && IN_BOUNDARIES(memalloc) &&
           IN_BOUNDARIES(memfree) && IN_BOUNDARIES(same_case_insensitive);
}
