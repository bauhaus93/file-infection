  ...
  while (next_instruction(&disasm)) {
    ...
    if (is_return(instr)) {
      ...
      if (is_conditional_jump(instr)) {
        ...
        block_list = check_block(instr->end, block_list, min_addr, max_addr);
      ...
      if (target != NULL && target >= min_addr && target < max_addr) {
        ...
        if (target_block == NULL) {
          block_list = check_block(target, block_list, min_addr, max_addr);
...
static ReferenceList *collect_references(const BlockList *blocks,
  ...
  for (const BlockList *ble = blocks; ble != NULL; ble = ble->next) {
    references = collect_references_from_block(ble->block, min_addr, max_addr,
                                               references);
  }
  return references;
}
